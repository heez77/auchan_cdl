...
def main(old_version=old_version):
    ...
    if old_version== None:
        ...
        MODEL_PATH = os.path.join(CFG.path_models,'Efficient_Det_surgele', 'Efficient_Det_surgele_v{}'.format(version))
        ...
        model = EfficientDetModel(num_classes=len(dico))
        MODEL_PATH = os.path.join(CFG.path_models,'Efficient_Det_surgele', 'Efficient_Det_surgele_v{}'.format(old_version))
        model.load_state_dict(torch.load(MODEL_PATH))
    model.eval()
    ...
    OUTPUT_DIR = os.path.join(CFG.path, 'Resultats', 'Surgele', 'resultat_surgele_{}.csv'.format(date))
